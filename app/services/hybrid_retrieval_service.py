...
def hybrid_retrieve(question: str) -> tuple[list[dict], dict]:
    ...
    logger.info(
        "Vector search: %d candidates in %.4fs",
        dense_count, vector_time,
    )

    t2 = time.perf_counter()
    bm25_results = bm25_search(question, top_k=settings.BM25_TOP_K)
    bm25_count = 0
    ...
    logger.info(
        "BM25 search: %d candidates in %.4fs",
        bm25_count, bm25_time,
    )
    ...
    logger.info("Total retrieval time: %.4fs", total)
